...
import { NativePlatform, PlatformType } from '@/lib/client/mobile';
...
export function PlatformTypeRenderer({ children, platform }: PlatformTypeProps) {
  ...
  return platformType === platform ? <>{children}</> : null;
}
...
export function DetailedPlatformRenderer({ children, platform }: DetailedPlatformProps) {
  const { platformType, nativePlatform } = usePlatform();

  if (platform === 'web') {
    return platformType === 'web' ? <>{children}</> : null;
  }
